...
description: Generic (language-agnostic) test-writer track of the tdd pipeline. Dispatched by the tdd orchestrator for a slice that is neither clearly backend nor frontend, to author spec-driven failing tests while blind to the implementation. Pipeline-internal; not for direct or automatic use.
