...
Exact flags drift between versions — confirm against the installed CLI. Schemas ship under `assets/` (`verdict.schema.json` post-green **and** sole-mode RED arbitration, `spec-adversary.schema.json` pre-freeze). Whatever the form, what you carry upward is a verdict matching that schema — and you note **which provider** produced it.
