...
**CI parity — local-green must equal CI-green (v0.39 §5.3).** When the validation contract declares a `ci_equivalent` (whole-tree) command for a check, run **both** the local (per-file) form and the CI-equivalent form through `scripts/ci-parity.py`. The live divergence this closes: CI runs `biome` format-only over the WHOLE tree while the local contract lints PER-FILE, so a slice green locally can be red in CI (`parallax-errors.md:88`). A slice whose local check passes but whose declared CI-equivalent form **fails** is **NOT green** — route the failure to its fault side; "green locally, red in CI" is a false green for shipping.
