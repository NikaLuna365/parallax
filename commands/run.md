...
1. Re-fetch `origin/<epic>` and re-run the **provenance** check (a resume must still start from the fresh remote tip — Step 0.6), then rebuild/verify the per-slice worktrees.
2. **Fail fast if still limited:** try one cheap operation; if the limit is still in force, just re-checkpoint `paused-on-limit` and exit — don't burn quota idling.
3. Otherwise continue from the checkpoint: skip `integrated` slices; for a `green-unverified` slice run **only** the owed Codex verification (don't rebuild it); resume `in_progress` slices from their last committed point; dispatch `pending` slices as their deps integrate. Idempotent — nothing already done is redone.
4. When the last slice integrates, set `status = complete`.
