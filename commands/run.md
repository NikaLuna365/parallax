...
- **GREEN** (all checks pass, pristine, no gaming, every declared integration seam resolves from its entry point) → **then the cross-model verifier, if enabled.** Read `.parallax/codex.toml`; if `enabled` and `points` includes `post_green`, dispatch `codex-judge` on this assembled slice *before* committing:
  ...
  - **Codex `concerns`** (the verifier flagged findings) → **do NOT commit yet.** The disposition is **mechanical and producer-proof** — Claude never authors the ledger or decides green by hand:
    ...
    python3 scripts/triage.py "$LEDGER" --policy .parallax/codex.toml --current-diff "$DIFF"; case $? in   # 0 green / 1 block / 2 escalate
      0) git add -A && git commit -q -m "$SID: green (low advisories recorded)";;
    ...
    ```
    Why this can't be gamed: `merge-ledger.py` is the **only** writer of findings (it maps the verifier's review round into the ledger by fingerprint — Claude invents no `id`/`spec_ref`/`evidence`); `triage.py` reads the `[review]` policy **only** from the trusted `.parallax/codex.toml` (never the ledger), and a `fixed` finding counts **only** if the verifier verified it (`verified_by=codex`) against the **current** `--current-diff`. Then act on the decision:
       ...
       - **`green`** (only `low` advisories remain, or every blocker is a codex-verified fix) → commit; advisories go to the run report (and verbose Telegram), not to a block.
