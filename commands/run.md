...
```bash
# (d0) STANDALONE FINALIZE GATE (v0.37 P0.2 + P1.5): per-slice green arbiter receipts + committed evidence
#      + no green-unverified slice + fresh run-state, then delegates the deep checks to epic-gate.py.
if ! python3 scripts/finalize-gate.py --feature-ref "$VERIFIED_OID" --slug "$SLUG"; then
  echo "HOLD: finalize gate failed (missing arbiter receipt / evidence / a green-unverified slice / stale run-state) — feature pushed for review; epic NOT advanced. Parking an epic-hold escalation."
  exit 0
fi
