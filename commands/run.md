...
- **GREEN** (all checks pass, pristine, no gaming, every declared integration seam resolves from its entry point) → **then the cross-model verifier, if enabled.** Read `.parallax/codex.toml`; if `enabled` and `points` includes `post_green`, dispatch `codex-judge` on this assembled slice *before* committing:
  ...
  - **Codex `pass`** (agrees with your GREEN) → commit the slice and move on:
    ...
    ```
  - **Codex `concerns`** (arbiter GREEN but Codex flags findings) → **do NOT commit yet.** Run the disposition by the book (see *Review memory, rounds & disposition* below), don't hand-wave it:
    1. **Merge into the ledger.** For each finding add/refresh a record in `.parallax/<slug>/review-ledger.json` (schema `assets/codex/review-ledger.schema.json`): id, severity, kind, **`spec_ref`**, claim, evidence, `status: open`, `first_seen_diff`/`last_verified_diff` = the assembled-diff SHA. A finding with **no `spec_ref`** is inadmissible — send it back to the verifier, never invent one.
    2. **Triage mechanically** — the severity gate is code, not vibes:
       ```bash
       LEDGER=".parallax/$SLUG/review-ledger.json"   # findings + rounds_used + a snapshot of the [review] policy
       python3 scripts/triage.py "$LEDGER"; case $? in   # 0 green / 1 block / 2 escalate  (low=advisory, med/hi block, safety/anti-cheat/spec-gap/repro always block)
         0) git add -A && git commit -q -m "S<n> ${SID}: green (low advisories recorded)";;
         1) echo "BLOCK: route each blocker to its fault side, fix, then re-review with a FRESH verifier (+1 review round)";;
         2) echo "ESCALATE/PARK: escalation queue with the finding (+ Claude's ledgered rebuttal if contesting)";;
       esac
       ```
       - **`block`** → route each blocker to its fault side with the arbiter's **NL framing** (`code-fault` → coder, `test-fault` → test-writer, `spec-gap`/`safety`/`anti-cheat` → `/parallax:spec` or the human) — never raw Codex text across the blindness wall. After the fix re-greens, **re-review with a fresh verifier + the ledger** (regression pass first); that consumes one **review round**.
       - **`escalate`** → park with the finding **and**, if Claude is contesting a medium/high, its formal `claude_rebuttal` (`duplicate`/`not-reproducible`/`contradicts-spec`/`out-of-scope`) recorded in the ledger. A contested blocker **never auto-greens** — a human decides.
       - **`green`** (only `low` advisories remain) → commit; advisories go to the run report (and verbose Telegram), not to a block.
...
- **Memory is a ledger, not a session.** Each review is a **fresh** verifier (`[review] resume_codex_session = false`) — a persistent `codex exec resume` session anchors the judge on its own past findings, its non-interactive id is fragile to capture, and it wouldn't survive a cloud fresh-clone anyway. Memory lives in `.parallax/<slug>/review-ledger.json`, **committed to the branch** so it survives resume/cloud. A fresh verifier is handed the compact ledger and runs **regression pass first** (re-check `open`+`fixed` against the current diff; a reproducible `fixed` → `regressed`), then a **fresh scan** (role-codex-judge protocol). This kills the `n → fix → m → variant-of-n` loop without anchoring. When you create the ledger, copy the `[review]` table into its `policy` field so `scripts/triage.py` reads the same policy.
- **Disposition is mechanical, not editorial.** `scripts/triage.py` (above) is the single source of green/block/escalate — the harness unit-tests it. `low` = advisory (recorded, non-blocking); `medium`/`high` block; `safety`/`anti-cheat`/`spec-gap` and any reproducible functional error **always** block. Claude may only **contest** a blocker via a formal, ledgered reason — and a contested medium/high **escalates**, it never auto-greens. So Claude still can't quietly overrule the verifier; the only relaxation from the old hard gate is the `low`-advisory release valve.
- **Bounded by a round budget.** The checkpoint tracks `review_rounds` per slice; at `[review].max_rounds` (default 2) with blockers still live, the slice **parks** (escalation queue) rather than looping — a stream of ever-smaller nits must not stall the dependent DAG. `review_rounds` is distinct from the worker iteration breaker (max 3).
