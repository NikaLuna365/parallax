...
9. **Pre-freeze gate.** Two parts — an independent cross-model review, then the gate itself.
   ...
   - **Bounded pre-freeze loop — executable, not interpretive.** Pre-freeze has its own fail-closed budget, `[review].pre_freeze_max_rounds` (default 2; falls back to `max_rounds` for an older config). Its single source of truth is `.parallax/<slug>/reviews/pre-freeze-state.json`, written **only** by `scripts/pre-freeze-budget.py`. Before **every** verifier dispatch, run `check`; if it returns `checkpoint`/exit 2, do not dispatch another reviewer. After a provider returns, pass its **verbatim schema-valid JSON** to `record`; the script writes the canonical `pre_freeze.round<N>.json`, counts severities, pins the review-policy hash, and refuses an unvalidated or unauthorized round. Claude-authored summaries are not review receipts. **(v0.37.3 F3)** `record` is also the single writer of the state's `closure`: it flips `closure.status` to `independent-pass` automatically — and exclusively — when the round's own verdict is `pass`; after a `concerns` round it stays `open`. There is no command, flag, or editable field to close pre-freeze any other way.
     ...
     MODE="interactive"   # v0.38 5.1: "autonomous" ONLY when this run was invoked --autonomous --from-doc.
     ...
     ```
     **(v0.38 5.1 / A2 — mechanical, not prose.)** `grant-one` refuses outright under `--mode autonomous`: a human round-grant is an interactive affordance, and an autonomous run parks at the cap instead. A grant hand-edited into an autonomous state fails on the very next read (`validate_state_semantics`).
   ...
   - **The gate — branch selection is BOUND to the run's real mode (v0.38 5.1, mechanical).** The v0.37.4 live audit caught exactly the bypass this closes: an `--autonomous --from-doc` run froze through the interactive human-OK branch, with closure honestly `open`, because a human happened to be at the console. Which branch below applies is **not a choice** — it is derived from how the run was invoked, pinned into `pre-freeze-state.json mode.autonomous` at init, and enforced by `freeze-check` (step 10). A human being present never converts an autonomous run into an interactive one.
     ...
     - *Autonomous (`--autonomous --from-doc <brief>` together — never `--from-doc` alone):* there is no human OK — the machine self-review (step 8) **and** the **independently closed** Codex pre-freeze review **are** the gate. Freeze only if the self-review is clean **and** `pre-freeze-state.json` records `closure.status = "independent-pass"` (v0.37.3 — written exclusively by `scripts/pre-freeze-budget.py record` when that round's own schema-valid verdict is `pass`; see *Pre-freeze closure, mechanically* below). Resolving findings into the spec/decision-log is necessary work but is **not** closure — only the next verifier round's `pass` is. A `concerns` verdict sitting at the round cap is a `checkpoint`, never a freeze — **and it stays a checkpoint even with a human at the console: the interactive human-OK branch is mechanically unreachable for an autonomous run (`freeze-check` refuses, exit 2 → park to the escalation queue).** Any unresolved `high`/`safety` finding blocks the freeze → escalation queue (`.parallax/<slug>/escalations.md`); autonomy never ships a spec it couldn't certify. If `codex` is unavailable, honor `on_missing` from the config — **with the v0.38 correction that neither `warn` nor anything else licenses an autonomous freeze without `independent-pass`:** `refuse` parks as before, and `warn` may stamp artifacts `UNVERIFIED` for *reporting*, but the autonomous freeze itself still fails closed at `freeze-check` (park/escalate; a human can relaunch the work interactively as a NEW, honestly-interactive run).
...
10. **Freeze.** On a passed gate only (interactive — including plain `--from-doc`: explicit human OK; autonomous `--autonomous --from-doc`: clean self-review **and** `closure.status = "independent-pass"` in `pre-freeze-state.json` — v0.37.3 F3):
    - **Run the mechanical freeze gate FIRST — no freeze happens without its exit 0 (v0.38 5.1, gate A1):**
      ```bash
      python3 scripts/pre-freeze-budget.py freeze-check "$STATE" --policy "$POLICY" --slug "$SLUG" --mode "$MODE" \
        || { echo "PARK: freeze refused — autonomous requires closure.status=independent-pass; the interactive branch is unreachable for an autonomous run (v0.38 5.1)"; exit 2; }
      ```
    ...
    - **Pin the review budget in force (v0.38 5.2, gate A3):**
      ```bash
