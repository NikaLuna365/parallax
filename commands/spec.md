...
- **The human OK gate (step 9) is replaced by the cross-model pre-freeze review.** Freeze only on a clean self-review **and** a recorded `closure.status = "independent-pass"` in `pre-freeze-state.json` (v0.37.3 F3 — a schema-valid verifier `pass` written by `scripts/pre-freeze-budget.py`; resolving findings and re-checking them yourself is preparation for the next round, never the closure itself, and a `concerns` at the cap stays a checkpoint). Any unresolved `high`/`safety` finding blocks the freeze → escalation queue. If the verifier is disabled or `codex` is unavailable, honor `on_missing` from `.parallax/codex.toml` (`refuse` — don't freeze autonomously without a verifier; or `warn` — freeze but stamp the artifacts `UNVERIFIED — human review required`).
