---
name: spec
description: "Phase 1 of the tdd pipeline. Drive a READ-ONLY interactive brainstorm into ONE maximally-concrete spec with zero open questions, plus a slice manifest and a real validation contract, then freeze them at a gate — a human OK, or in autonomous mode (--from-doc) a machine self-review plus an independent Codex pre-freeze review. Run this before /tdd:run."
argument-hint: "<what you want to build>   |   --autonomous --from-doc <brief-path>"
---

# /tdd:spec — turn an idea into a frozen, build-ready spec

You are running **Phase 1** of the tdd pipeline for: **$ARGUMENTS**

Your job is to produce, through collaborative dialogue, three frozen artifacts and nothing else:

- `.tdd/<feature-slug>/spec.md` — the single, READ-ONLY source of truth (WHAT to build, not how to test or code it).
- `.tdd/<feature-slug>/slices.md` — the slice manifest (how the work splits, and which agent pair owns each piece).
- `.tdd/<feature-slug>/validation.md` — the validation contract (the project's REAL test/lint/build commands + path globs).

These live under a **per-feature subdirectory** `.tdd/<feature-slug>/`, never the `.tdd/` root. The reason is purely mechanical: two feature branches that both wrote `.tdd/spec.md` would collide add/add on that exact path at merge time, so the second PR in the queue is always red through no fault of its own. Namespacing by slug removes that class of conflict by construction. You derive the slug at the Freeze step.

After this, `/tdd:run` executes the build. You do **not** run it — you stop at the freeze gate.
...
This is stricter than ordinary brainstorming, and the reason is structural. In `/tdd:run`, a **blind test-writer** and a **blind coder** work from this spec **independently** — the coder never sees the tests, the test-writer never sees the code. They can only converge if the spec is unambiguous. **Every ambiguity you leave becomes a test↔code divergence later, which the arbiter must escalate as a spec-gap — the most expensive failure mode.** So an ambiguity check here is not a stylistic nicety; it is the core deliverable. If a requirement could be read two ways, pin one — explicitly, with an example.

## Hard rules

- **READ-ONLY on the codebase.** In this phase you may read/explore anything, but you may **not** create or modify any source or test file. The only files you write are the three `.tdd/<feature-slug>/` artifacts. The only git you do is creating the feature branch and committing those artifacts.
- **Do not start implementation or invoke `/tdd:run`.** Stop at the OK gate. This applies to every feature regardless of perceived simplicity — "too simple to spec" is exactly where unexamined assumptions cause wasted work.
...
7. **Build the validation contract.** From the toolchain, detect the project's real commands and **confirm each with the user** (never invent or weaken them — a made-up command that "passes" is the documented cause of false-green completions). Fill the **Validation contract format** below, including the **source and test path globs** — `/tdd:run` uses these to physically hide `tests/` from the coder and `src/` from the test-writer (the blindness mechanism). If a command genuinely can't be discovered, record it as an open item and resolve it with the user — don't guess. Also capture **Provisioning**: the gitignored build deps a fresh worktree won't have (`node_modules`, generated clients like `prisma generate`) and how to supply them — `/tdd:run` runs these in every worktree it creates, and without them the done-gate fails spuriously (a fresh checkout has no `node_modules`, so even correct code won't compile).
...
- **Everything else is identical:** READ-ONLY on the codebase, YAGNI, the per-feature `.tdd/<slug>/` artifacts, the freeze + branch creation. Autonomy changes *who decides*, never *what a good spec is*.

---

## Spec format (`.tdd/<feature-slug>/spec.md`)
...
## Slice manifest format (`.tdd/<feature-slug>/slices.md`)
...
## Validation contract format (`.tdd/<feature-slug>/validation.md`)
...
- Shared/always-present: `.tdd/**` plus config the build needs (manifests, tsconfig, etc.)
