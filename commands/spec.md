---
name: spec
description: "Phase 1 of the Parallax pipeline. Drive a READ-ONLY interactive brainstorm into ONE maximally-concrete spec with zero open questions, plus a slice manifest and a real validation contract, then freeze them at a gate — a human OK, or in autonomous mode (--autonomous --from-doc, both flags together; --from-doc alone stays human-gated) a machine self-review plus an independent Codex pre-freeze review whose schema-valid pass is the ONLY thing that can close pre-freeze (v0.37.3: orchestrator/human self-attestation never certifies a freeze). Run this before /parallax:run. With --from-doc it accepts a structured Parallax Brief Packet (or unstructured markdown) and, if the brief is not build-ready, returns a bounded Intake Response with blocking questions instead of freezing or building."
...
- **Mode.** The rules and steps below describe the default **interactive** mode. **Autonomous mode is the flag COMBINATION `--autonomous --from-doc <brief>` — never `--from-doc` alone (v0.37.3 F3).** Only that combination replaces the interactive touchpoints — one-question-at-a-time (step 3) and the human OK gate (step 9) — as **Autonomous mode** specifies; everything else (READ-ONLY, YAGNI, the full self-review, the frozen artifacts) holds identically. Plain `--from-doc` without `--autonomous` is the **intake/brief input path**: the brief seeds the brainstorm instead of a typed request, but step 3's questioning and step 9's explicit human OK apply exactly as interactive mode — a written brief is never, by itself, a licence to skip the human. (Before v0.37.3 this line said just "`--from-doc`", which a live run could — and did — read as authorizing the no-human-OK path without `--autonomous`; it does not.)
...
3.6. **Prohibition Reconciliation (read-only — record what must NOT be violated).** The affordance review (3.5) finds what to *reuse*; this sibling pass records what to *avoid* — the concrete prohibitions and invariants this feature could break once its slices are assembled, which no per-slice green will catch. Grep/read the **local gate tests, forbidden-pattern lists, invariant files, ADRs, and validation contracts** in the touched area: existing `*.test.*` that assert an *absence*, `FORBIDDEN`/denylist constants, money/PII/anti-cheat rules, and the serialization contracts of shared entities. Record the machine-checkable ones in **`.parallax/<slug>/invariants.json`** (`assets/feature-invariants.schema.json`):
   - `forbidden_patterns` — a regex that must never reach shipped code under given globs (e.g. a PII field serialized onto the wire, a forbidden money literal);
   - `required_consumers` — a new/changed shared field must have a **live consumer**, not just a producer or a unit-test mention (else it is a dead field/seam);
   - `mock_only_slices` — an I/O-heavy slice must ship an integration/contract check or carry the explicit `externals mocked -> integration unverified` stamp.

   `/parallax:run` Step 3 runs `scripts/feature-sweep.py` against this manifest before completion, so the prohibitions are enforced mechanically, not just remembered. Record only invariants you actually verified in the touched area; an empty manifest is allowed only if you genuinely found none and say so.
...
9. **Pre-freeze gate.** Two parts — an independent cross-model review, then the gate itself.
   ...
   - **Bounded pre-freeze loop — executable, not interpretive.** Pre-freeze has its own fail-closed budget, `[review].pre_freeze_max_rounds` (default 2; falls back to `max_rounds` for an older config). Its single source of truth is `.parallax/<slug>/reviews/pre-freeze-state.json`, written **only** by `scripts/pre-freeze-budget.py`. Before **every** verifier dispatch, run `check`; if it returns `checkpoint`/exit 2, do not dispatch another reviewer. After a provider returns, pass its **verbatim schema-valid JSON** to `record`; the script writes the canonical `pre_freeze.round<N>.json`, counts severities, pins the review-policy hash, and refuses an unvalidated or unauthorized round. Claude-authored summaries are not review receipts. **(v0.37.3 F3)** `record` is also the single writer of the state's `closure`: it flips `closure.status` to `independent-pass` automatically — and exclusively — when the round's own verdict is `pass`; after a `concerns` round it stays `open`. There is no command, flag, or editable field to close pre-freeze any other way.
     ```bash
     STATE=".parallax/$SLUG/reviews/pre-freeze-state.json"
     POLICY=".parallax/codex.toml"
     MODE="interactive"   # v0.37.5 5.1: "autonomous" ONLY when this run was invoked --autonomous --from-doc.
                          # Pinned into the state at init; every later call must match (a relabel = GateError).
     python3 scripts/pre-freeze-budget.py check "$STATE" --policy "$POLICY" --slug "$SLUG" --mode "$MODE"
     ...
     python3 scripts/pre-freeze-budget.py record "$STATE" "$RAW_VERDICT" \
       --policy "$POLICY" --slug "$SLUG" --mode "$MODE" --provider "$PROVIDER" \
     ...
     python3 scripts/pre-freeze-budget.py grant-one "$STATE" --policy "$POLICY" \
       --slug "$SLUG" --mode "$MODE" --token "$HUMAN_REPEATED_GRANT_TOKEN"
     ```
     **(v0.37.5 5.1 / A2 — mechanical, not prose.)** `grant-one` refuses outright under `--mode autonomous`: a human round-grant is an interactive affordance, and an autonomous run parks at the cap instead. A grant hand-edited into an autonomous state fails on the very next read (`validate_state_semantics`).
   ...
   - **The gate — branch selection is BOUND to the run's real mode (v0.37.5 5.1, mechanical).** The v0.37.4 live audit caught exactly the bypass this closes: an `--autonomous --from-doc` run froze through the interactive human-OK branch, with closure honestly `open`, because a human happened to be at the console. Which branch below applies is bound to the **recorded** mode: `--mode` is declared once at the first budget call (honestly derived from the invocation flags — mis-declaring it is a contract violation this text cannot mechanically prevent), pinned into `pre-freeze-state.json mode.autonomous` at init, immutable thereafter (a relabel is a GateError), and enforced by `freeze-check` (step 10). What IS mechanical: once a run has ever identified as autonomous, no human at the console, no `on_missing=warn`, and no relabel can reach the interactive human-OK branch — the RUN1 side door. The initial declaration itself is a directive obligation, stated here honestly.
     - *Interactive (default — and this includes plain `--from-doc` with no `--autonomous`, v0.37.3 F3):* present the full set — spec + slice manifest + validation contract, plus any Codex findings and how you resolved them — and get an explicit human OK. Do not proceed on silence or a vague "looks fine". If the user requests changes, revise and re-run the self-review. A brief-seeded run (`--from-doc` alone) stops at this same human OK; only `--autonomous --from-doc` takes the branch below.
     - *Autonomous (`--autonomous --from-doc <brief>` together — never `--from-doc` alone):* there is no human OK — the machine self-review (step 8) **and** the **independently closed** Codex pre-freeze review **are** the gate. Freeze only if the self-review is clean **and** `pre-freeze-state.json` records `closure.status = "independent-pass"` (v0.37.3 — written exclusively by `scripts/pre-freeze-budget.py record` when that round's own schema-valid verdict is `pass`; see *Pre-freeze closure, mechanically* below). Resolving findings into the spec/decision-log is necessary work but is **not** closure — only the next verifier round's `pass` is. A `concerns` verdict sitting at the round cap is a `checkpoint`, never a freeze — **and it stays a checkpoint even with a human at the console: the interactive human-OK branch is mechanically unreachable for an autonomous run (`freeze-check` refuses, exit 2 → park to the escalation queue).** Any unresolved `high`/`safety` finding blocks the freeze → escalation queue (`.parallax/<slug>/escalations.md`); autonomy never ships a spec it couldn't certify. If `codex` is unavailable, honor `on_missing` from the config — **with the v0.37.5 correction that neither `warn` nor anything else licenses an autonomous freeze without `independent-pass`:** `refuse` parks as before, and `warn` may stamp artifacts `UNVERIFIED` for *reporting*, but the autonomous freeze itself still fails closed at `freeze-check` (park/escalate; a human can relaunch the work interactively as a NEW, honestly-interactive run).
   - **Pre-freeze closure, mechanically (v0.37.3 F3).** The live-run failure this closes: the pre-freeze gate hit its round budget on a `concerns` verdict and the run was then treated as review-complete on the orchestrator's own `all_resolved: true` prose — self-attestation standing in for independent re-verification. Now the closure state is machine-written and fail-closed:
     - `pre-freeze-state.json` carries a required `closure` object (schema: `assets/codex/pre-freeze-state.schema.json`) whose `status` enum has exactly two values — `open` and `independent-pass`. There is **no** self-attested / orchestrator-resolved / human-approved closure status, so that trust level cannot even be represented, let alone accepted.
     - Only `scripts/pre-freeze-budget.py record` moves closure to `independent-pass`, and only when the round it is recording carries a schema-valid verifier verdict of `pass`. A `concerns` round — including the one that exhausts the budget — leaves closure `open` and the decision `checkpoint`. No flag, field, note, or free-text summary converts a checkpoint into closed; the script validates on every read that an `independent-pass` closure actually points at the **last recorded round** and that its verdict really was `pass`, so a hand-edited state file fails the gate instead of certifying it.
     - A human's explicit `grant-one` (repeating the exact `grant_token`) authorizes **one more verifier round only**. The grant itself certifies nothing: closure still becomes `independent-pass` only if that granted round returns `pass`.
     - The freeze gates above consume this mechanically: autonomous freeze requires `closure.status = "independent-pass"`; interactive freeze still requires the explicit human OK *on top of* whatever the review state says.

10. **Freeze.** On a passed gate only (interactive — including plain `--from-doc`: explicit human OK; autonomous `--autonomous --from-doc`: clean self-review **and** `closure.status = "independent-pass"` in `pre-freeze-state.json` — v0.37.3 F3):
    - **Run the mechanical freeze gate FIRST — no freeze happens without its exit 0 (v0.37.5 5.1, gate A1):**
      ```bash
      python3 scripts/pre-freeze-budget.py freeze-check "$STATE" --policy "$POLICY" --slug "$SLUG" --mode "$MODE" \
        || { echo "PARK: freeze refused — autonomous requires closure.status=independent-pass; the interactive branch is unreachable for an autonomous run (v0.37.5 5.1)"; exit 2; }
      ```
      The check reads only artifacts (the pinned `mode` + `closure`), so a human at the console, an `on_missing=warn` config, or a missing state file cannot open the interactive side door for an autonomous run. Interactive runs get `freeze_path: interactive-human-ok` and must still collect the explicit human OK.
    - **Pin the review budget in force (v0.37.5 5.2, gate A3):**
      ```bash
      python3 scripts/pre-freeze-budget.py pin-policy --policy "$POLICY" --slug "$SLUG" \
        --out ".parallax/$SLUG/review-policy.frozen.json"
      ```
      This freezes `[review] max_rounds` (+ the full triage policy and its `policy_hash`) at freeze time, committed with the contract below. From here every rounds-budget decision (`merge-ledger.py`, `triage.py`, `epic-gate.py`) evaluates against the **pinned** snapshot — a later `codex.toml` edit changes no gate verdict, and widening the budget is sanctioned only by a recorded review-budget amendment (`contract-amend.py`).
    ...
    - Commit them to `feature/<slug>` **together with `.parallax/<slug>/review-policy.frozen.json`** (these artifacts live in `.parallax/<slug>/`, which every worktree keeps; only the project's `src/`/`tests/` get sparsely hidden).
...
- **The human OK gate (step 9) is replaced by the cross-model pre-freeze review — and the replacement is mechanically BOUND to the mode (v0.37.5 5.1).** Freeze only on a clean self-review **and** a recorded `closure.status = "independent-pass"` in `pre-freeze-state.json` (v0.37.3 F3 — a schema-valid verifier `pass` written by `scripts/pre-freeze-budget.py`; resolving findings and re-checking them yourself is preparation for the next round, never the closure itself, and a `concerns` at the cap stays a checkpoint). `freeze-check --mode autonomous` enforces this before any freeze: a human at the console does **not** re-open the interactive branch for an autonomous run — if the verifier couldn't certify the spec, the run parks (escalation queue) and a human may relaunch it as a NEW, honestly-interactive run. Any unresolved `high`/`safety` finding blocks the freeze → escalation queue. If the verifier is disabled or `codex` is unavailable: `on_missing = refuse` parks as before; **`warn` no longer licenses an autonomous freeze** (v0.37.5 — it may stamp reporting artifacts `UNVERIFIED`, but `freeze-check` still fails closed without `independent-pass`).
...
| S2 | <one purpose> | frontend | test-writer-frontend + blind-coder-frontend | S1 | <consumes S1's `createUser` from `@app/users`; seam reachability: user-reachable — new Users screen opened via the Settings tab> |

Frontend seams carry a **reachability class** (v0.37.3 F2): `internal/import-only` | `route-registered` | `user-reachable`. It sets the arbiter's proof bar — `user-reachable` demands an interaction test (drive the entry affordance, assert the destination content appears), never router membership alone.
```
...
- Monorepo dependency roots (v0.37.3 F1 — record ONLY when the touched package needs a SIBLING package's source or built output present just to resolve cross-package imports, e.g. a pnpm workspace importing `@scope/shared`): `<package-specific globs, e.g. packages/shared/src/**, packages/shared/dist/**, or "none">`. `/parallax:run` folds these into the per-slice scope manifest for `scripts/blindfold-guard.py --scope-manifest`, so the test-writer's worktree can keep what import resolution genuinely needs while the slice's OWN new/changed implementation still fails closed. Name specific package roots only — never the slice's own package, and never a whole-tree `**` (the manifest schema rejects all-wildcard globs; a blanket allowlist is not a monorepo mode).
...
- record any session/transcript path **only** under `provenance.transcript_path` as **auxiliary provenance** — never as primary proof; if unavailable, set it `null` and add an `evidence_limits` note. **(v0.37.3 P2)** Keep that note factual: "not captured by this run" is honest; "unavailable in this harness" asserted while the transcript actually exists on disk is an evidence defect — record the real path when it exists.
