...
Source: `ANALYSIS_v0.37.4_live_production_runs.md` + `TRIAGE_v0.37.4_live_runs_to_v0.37.5.md` (two real
...
passed). These entries extend the F-series with the shapes v0.37.3 did not cover — remediated in v0.37.5:
...
- **F3 NEW-MODE — freeze-gate mode binding (RUN1).** An `--autonomous` run froze via the interactive
  ...
  branch selection was bypassable by a human being present. → v0.37.5 5.1 (`freeze-check`, pinned
...
- **F3-family — post-green budget authority (RUN1).** A rounds-exceeded epic-gate HOLD was cleared by
  ...
  authorized by a bare `assumption_recorded`. → v0.37.5 5.2 (pinned budget + review-budget amendments).
- **F3-family / F4-F6-adjacent — post-green receipt integrity (both runs).** No post-green raw
  ...
  envelope after the parser crashed. → v0.37.5 5.3 (mandatory raw receipts, schema-valid rounds only).
- **F7 NEW-MODE — resume-state drift (RUN2).** `run-state.json`/`RUN-HANDOFF.md` recorded S6-test at
  ...
  no write-back and no `session_handoff` event. → v0.37.5 6.1 (`resume-reconcile.py`, per-round
  write-back).
- **F2-adjacent (LATENT, RUN2).** The S5↔S6 19MB media-gate was "proven" by a test-authored duplicate
  normalizer, not the production path; `invariants.json` had no normalize→gate consumer. → v0.37.5 6.2
  (production-path consumers, arbiter rule).
- **F5 HELD-PARTIAL / telemetry (both).** Uneven per-slice iteration events, no sweep receipt,
  zero-width/inert leases, stale spec-phase run-evidence, transcript_path → directory. → v0.37.5 D1–D3.
- **F6 NEW-MODE (RUN2).** OpenAI strict mode also demands full `required` enumeration; the judge
  hand-tuned schemas across 3–4 calls per slice. → v0.37.5 E1 (fully strict shim + normalize).
- **F1 — still NOT-fully-EXERCISED.** Both runs single-package; the cross-package sibling case remains
  fixture-proven only. → v0.37.5 soak requirement (gate F1g), not a code change.
