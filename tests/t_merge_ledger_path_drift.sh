...
JSON
python3 "$M" "$L" "$T/r1.json" --slice S1 --current-diff d1 --slug demo --repo-root "$T/repo" >/dev/null || fail "1: round1 merge failed"
...
JSON
python3 "$M" "$L" "$T/r2.json" --slice S1 --current-diff d2 --slug demo --repo-root "$T/repo" >/dev/null || fail "1: round2 merge failed"
...
JSON
python3 "$M" "$L" "$T/r3.json" --slice S1 --current-diff d3 --slug demo --repo-root "$T/repo" >/dev/null || fail "2: round3 merge failed"
...
JSON
python3 "$M" "$L" "$T/r4.json" --slice S1 --current-diff d4 --slug demo --repo-root "$T/repo" >/dev/null || fail "3: round4 merge failed"
...
LA="$T/ledger_amb.json"
cat > "$T/a1.json" <<'JSON'
{"verdict":"concerns","findings":[
  {"severity":"medium","kind":"code-fault","spec_ref":"B1","where":"a/dup.ts:1","claim":"c1","evidence":"e1"}]}
JSON
python3 "$M" "$LA" "$T/a1.json" --slice S1 --current-diff d1 --slug demo --repo-root "$T/repo" >/dev/null 2>&1 || fail "4: ambiguous round1 failed"
cat > "$T/a2.json" <<'JSON'
{"verdict":"pass","findings":[],"resolved":[
  {"kind":"code-fault","spec_ref":"B1","where":"dup.ts:1","note":"claims fixed"}]}
JSON
OUT=$(python3 "$M" "$LA" "$T/a2.json" --slice S1 --current-diff d2 --slug demo --repo-root "$T/repo" 2>/tmp/parallax_mlpd_err) || fail "4: ambiguous round2 failed"
...
LB="$T/ledger_id.json"
python3 "$M" "$LB" "$T/r1.json" --slice S1 --current-diff d1 --slug demo --repo-root "$T/repo" >/dev/null || fail "6: seed failed"
...
JSON
python3 "$M" "$LB" "$T/r6.json" --slice S1 --current-diff d2 --slug demo --repo-root "$T/repo" >/dev/null || fail "6: cited-id merge failed"
...
PY
LB2="$T/ledger_id2.json"
python3 "$M" "$LB2" "$T/r1.json" --slice S1 --current-diff d1 --slug demo --repo-root "$T/repo" >/dev/null
...
JSON
python3 "$M" "$LB2" "$T/r7.json" --slice S1 --current-diff d2 --slug demo --repo-root "$T/repo" >/dev/null 2>/dev/null
...
LC="$T/ledger_legacy.json"
python3 "$M" "$LC" "$T/r1.json" --slice S1 --current-diff d1 --slug demo >/dev/null || fail "7: legacy round1 failed"
python3 "$M" "$LC" "$T/r3.json" --slice S1 --current-diff d2 --slug demo >/dev/null || fail "7: legacy round2 failed"
