...
- **`/parallax:run [slug]`** *(`--autonomous` · `--parallel` · `--resume` · `--adopt`)* — build each slice blind, arbitrate to green, integrate, push. `--resume` continues a clean limit-pause; `--adopt` recovers an **uncleanly**-interrupted run (session/context death mid-build) git-first from `subagents.json` + the reconciled checkpoint, failing closed on irreconcilable state.
